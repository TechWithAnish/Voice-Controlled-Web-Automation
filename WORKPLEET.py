...
from PyQt5.QtWidgets import QApplication, QPushButton, QMainWindow, QAction, QInputDialog, QFileDialog, QPushButton, \
...
class Crawler:
    ...
    def get_element_attributes(self, element_info):
        ...
        attr_dict = {attr: attrs[attr] for attr in
                     ['value', 'type', 'title', 'placeholder', 'name', 'aria-label', 'role'] if
                     attr in attrs and attrs[attr]}

        if tag in ['button', 'textarea', 'option'] or attrs.get('role') in ['button', 'checkbox',
                                                                            'radio'] or attr_dict.get('type') in [
        ...
        def extract_elements(script, target_page, iframe_id=None):
            ...
            for element_info in elements_in_viewport:
                ...
                if attr_dict and display != "none" and isVisible:
                    ...
                    attr_str = ' '.join(
                        [f'id="{attr_dict["id"]}"'] + [f'{key}="{value}"' for key, value in attr_dict.items() if
                                                       key not in ['id', 'role']])
                    ...
                    elif (tag == 'input' and attr_dict.get('type') != 'submit') or tag == 'textarea' or attr_dict.get(
                            'role') == 'textbox':
                    ...
                    if tag == 'iframe':
                        ...
                        if iframe_element:
                            ...
                            iframes_list.append(
                                (attr_dict["id"], iframe_content))  # Store the iframe and its ID in the list
    ...
    while True:
        ...
        def get_gpt_command(string_data):
            ...
            response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[{"role": "system", "content": prompt}, {"role": "user", "content": "YOUR ANSWER: "}])
        ...
        class CustomMainWindow(QMainWindow):
            ...
            def getResizeHandleAt(self, pos):
                ...
                if QRect(0, 0, handle_size, handle_size).contains(pos):
                    ...
                    return "BottomLeft"
                elif QRect(rect.width() - handle_size, rect.height() - handle_size, handle_size, handle_size).contains(
                        pos):
                    return "BottomRight"
                elif QRect(0, handle_size, handle_size, rect.height() - 2 * handle_size).contains(pos):
                    return "Left"
                elif QRect(rect.width() - handle_size, handle_size, handle_size,
                           rect.height() - 2 * handle_size).contains(pos):
                    return "Right"
                elif QRect(handle_size, 0, rect.width() - 2 * handle_size, handle_size).contains(pos):
                    return "Top"
                elif QRect(handle_size, rect.height() - handle_size, rect.width() - 2 * handle_size,
                           handle_size).contains(pos):
                    return "Bottom"
        ...
        def create_window():
            def load_file():
                ...
                file_path, _ = file_dialog.getOpenFileName(window, 'Select File', '',
                                                           'PDF Files (*.pdf);;DOC Files (*.doc *.docx);;CSV Files (*.csv)')
            ...
            input_button.setStyleSheet(
                "QPushButton { background-color: #ea9d59; color: black; font-family: 'Roboto-Mono'; font-weight: bold;  border-radius: 10px;}")
            ...
            url_input.setStyleSheet(
                "QLineEdit { background-color: black; border-radius: 15px; border: 1px solid #c8c8c8; font-size: 14px; font-family: 'Roboto Mono'; color: #ea9d59;}")
            ...
            submit_btn.setStyleSheet(
                "QPushButton { background-color: #ea9d59; color: black; font-weight: bold; font-family: 'Roboto-Mono'; border-radius: 10px;}")
